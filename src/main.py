...
# 定义模型列表
#ollama_models = ["llama3.1:8b-instruct-q4_0", "llama3-chinese:latest","qwen2.5:7b","gemma2:2b"]
...
def update_model_list(model_type):
    if model_type == "openai":
        return gr.Dropdown(choices=["gpt-4o-mini","gpt-4o","gpt-3.5-turbo" ], label="选择模型")
    elif model_type == "ollama":
        return gr.Dropdown(choices=["llama3.1:8b-instruct-q4_0", "llama3-chinese:latest","qwen2.5:7b","gemma2:2b"], label="选择模型")
...
with gr.Blocks(title="LanguageMentor 英语私教") as language_mentor_app:
    with gr.Tab("场景训练"):
        ...
        # 模型名称选择 (动态更新)
        model_name_dropdown = update_model_list("ollama")  # 初始化 Dropdown 为 ollama 的模型列表
